...
contract RegistryEntry is ApproveAndCallFallBack {
   ...
   * @param _challengeDescription Challenge comment
   */
  ...
  notEmergency
  {
    ...
    challenge.description = _challengeDescription;
   ...
   * Challenger has reward on0ly if winning option is VoteAgainst
   ...
   */
  function loadRegistryEntry() public constant returns (uint, Status, address, uint, uint, address, address, string, string) {
    return (
    version,
    status(),
    creator,
   ...
   */
  function loadRegistryEntryChallenge() public constant returns (uint, address, uint, string, uint, uint, uint, uint, uint, uint) {
    return (
    challengePeriodEnd,
